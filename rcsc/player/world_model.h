...
private:
    ...
    //! uniform number of teammate goalie
    int M_teammate_goalie_unum;
    ...
    AbstractPlayerCont M_all_teammates; //!< all teammates pointers includes self
    AbstractPlayerCont M_all_opponents; //!< all opponents pointers includes unknown
...
public:
    ...
    */
     ...
     */
    int teammateGoalieUnum() const { return M_teammate_goalie_unum; }

    /*!
      \brief get the uniform number of opponent goalie
      \return uniform number value or Unum_Unknown
     */
    int opponentGoalieUnum() const { return M_opponent_goalie_unum; }
    ...
    /*!
      \brief get all teammate playerss includes self.
      \return const rerefence to the AbstractPlayerObject pointer container.
     */
    const AbstractPlayerCont & allTeammates() const { return M_all_teammates; }

    /*!
      \brief get all opponent players (includes unknown players)
      \return const rerefence to the AbstractPlayerObject pointer container.
     */
    const AbstractPlayerCont & allOpponents() const { return M_all_opponents; }
    ...
    */
    int teammatePlayerTypeId( const int unum ) const
      {
          if ( unum < 1 || 11 < unum )
          {
    ...
    */
    int opponentPlayerTypeId( const int unum ) const
      {
          if ( unum < 1 || 11 < unum )
          {
     ...
     */
    Card teammateCard( const int unum ) const
      {
          if ( unum < 1 || 11 < unum )
          {
    ...
    /*!
      ...
      \return yellow card status
     */
    Card opponentCard( const int unum ) const
      {
          if ( unum < 1 || 11 < unum )
          {
    ...
    */
     ...
     */
    int countPlayer( const PlayerPredicate * predicate ) const;
    ...
    /*!
      ...
      \return number of players.
     */
    int countPlayer( boost::shared_ptr< const PlayerPredicate > predicate ) const;

    /*!
      \brief get teammate goalie pointer
    ...
    */
     ...
     */
    template < typename REGION >
    int countPlayersIn( const REGION & region,
                        const PlayerPtrCont & players,
                        const int count_thr,
                        const bool with_goalie ) const
      {

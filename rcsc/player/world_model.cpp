...
WorldModel::WorldModel()
    : M_localize( new LocalizationDefault() ),
      ...
      M_self(),
      M_ball(),
      M_teammate_goalie_unum( Unum_Unknown ),
                          ...
                          const bool my_goalie )
{
    ...
    if ( my_goalie )
    {
        M_teammate_goalie_unum = my_unum;
    }
...
WorldModel::teammatePlayerType( const int unum ) const
{
    ...
    const PlayerType * p = PlayerTypeSet::i().get( teammatePlayerTypeId( unum ) );
    if ( ! p )
    {
...
WorldModel::opponentPlayerType( const int unum ) const
{
    ...
    const PlayerType * p = PlayerTypeSet::i().get( opponentPlayerTypeId( unum ) );
    if ( ! p )
    {
...
WorldModel::updatePlayerStateCache()
{
    ...
    //
    if ( M_teammate_goalie_unum == Unum_Unknown )
    {
        const AbstractPlayerObject * p = getTeammateGoalie();
        if ( p )
        {
            M_teammate_goalie_unum = p->unum();
        }
    }
...
WorldModel::updateOurOffenseLine()
{
    ...
    const AbstractPlayerCont::const_iterator end = allTeammates().end();
    for ( AbstractPlayerCont::const_iterator it = allTeammates().begin();
          it != end;
          ++it )
    {
    ...
    if ( allTeammates().empty() )
    {
        // new_line is used
    }
    else if ( allTeammates().size() >= 11 )
    {
        // new_line is used
    }
...
WorldModel::updateOurDefenseLine()
{
    ...
    const AbstractPlayerObject * goalie = getTeammateGoalie();
    if ( ! goalie )
    {
...
WorldModel::updateTheirOffenseLine()
{
    double new_line = ServerParam::i().pitchHalfLength();
    {
        const AbstractPlayerCont::const_iterator end = allOpponents().end();
        for ( AbstractPlayerCont::const_iterator it = allOpponents().begin();
              it != end;
              ++it )
        {
...
WorldModel::getTeammateGoalie() const
{
